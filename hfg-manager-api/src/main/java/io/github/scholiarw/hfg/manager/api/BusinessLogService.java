...
final class BusinessLogService {
  ...
  void recordQuotaAfterCommit(UUID userId, TransferDirection direction, Instant windowStart, String status) {
  ...
  private void insertStarted(JdbcClient jdbc, TransferEvent e) {
    String sql = logs.vendor() == DatabaseDialect.Vendor.POSTGRESQL
  ...
  private void complete(JdbcClient jdbc, TransferEvent e) {
    Optional<Map<String,Object>> existing = jdbc.sql(
        ...
        .param("id", e.transferId()).query().listOfRows().stream().findFirst();
    ...
    if (existing.isPresent()) {
      jdbc.sql("update logs set status=:status,file_size_bytes=:bytes,ended_at=:ended,duration_millis=:duration,average_bytes_per_second=:speed,error_code=:error,updated_at=:ended where log_date=:day and record_type='TRANSFER' and log_id=:id")
          .param("status", e.status().name()).param("bytes", e.bytes())
          .param("ended", Timestamp.from(e.occurredAt())).param("duration", duration)
          .param("speed", speed).param("error", e.errorCode() == null ? null : e.errorCode().name())
          .param("day", existing.get().get("log_date")).param("id", e.transferId()).update();
      return;
    }
    String sql = logs.vendor() == DatabaseDialect.Vendor.POSTGRESQL
  ...
  private JdbcClient.StatementSpec bindEvent(JdbcClient.StatementSpec statement, TransferEvent e) {
    return statement.param("day", LocalDate.ofInstant(e.occurredAt(), ZoneOffset.UTC))
        .param("id", e.transferId()).param("user", e.userId()).param("username", username(e.userId()))
        .param("protocol", e.protocol().name()).param("direction", e.direction().name())
        .param("status", e.status().name()).param("path", e.virtualPath())
        .param("file", fileName(e.virtualPath())).param("at", Timestamp.from(e.occurredAt()))
        .param("gateway", e.gatewayId()).param("client", e.clientAddress())
  ...
  private void recordQuota(UUID userId, TransferDirection direction, Instant start, String status) {
    Map<String,Object> row = management.sql(
        ...
        .param("user", userId).param("direction", direction.name()).param("start", start)
        .query().singleRow();
    ...
    boolean reached = (fileLimit > 0 && files >= fileLimit) || (byteLimit > 0 && bytes >= byteLimit);
    Instant now = Instant.now();
    String sql = "insert into logs(log_date,record_type,log_id,user_id,username,direction,status,window_start,window_end,completed_files,completed_bytes,reserved_files,reserved_bytes,file_limit,byte_limit,quota_reached,created_at,updated_at) "
        + "values(:day,'QUOTA',:id,:user,:username,:direction,:status,:start,:end,:cf,:cb,:rf,:rb,:fl,:bl,:reached,:now,:now)";
    logs.jdbc().sql(sql).param("day", LocalDate.ofInstant(now, ZoneOffset.UTC)).param("id", UUID.randomUUID())
        .param("user", userId).param("username", row.get("username")).param("direction", direction.name())
        .param("status", status).param("start", row.get("window_start")).param("end", row.get("window_end"))
        .param("cf", row.get("completed_files")).param("cb", row.get("completed_bytes"))
        .param("rf", row.get("reserved_files")).param("rb", row.get("reserved_bytes"))
        .param("fl", fileLimit).param("bl", byteLimit).param("reached", reached)
        .param("now", Timestamp.from(now)).update();
  }

  private String username(UUID userId) {
    return management.sql("select username from ftp_user where id=:id").param("id", userId)
        .query(String.class).optional().orElse("<deleted>");
  }

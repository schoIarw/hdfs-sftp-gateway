create table hdfs_cluster (
  ...
  status varchar(32) not null default 'ENABLED', created_at timestamptz not null, updated_at timestamptz not null
);
...
create table gateway_node (
  ...
  software_version varchar(64), snapshot_version bigint not null default 0, last_heartbeat_at timestamptz,
  status varchar(32) not null default 'UNKNOWN', created_at timestamptz not null, updated_at timestamptz not null,
  unique(service_group_id,hostname)
);

create table ftp_user (
  id uuid primary key, username varchar(64) not null unique, password_hash varchar(255) not null,
  ...
  hdfs_effective_user varchar(128), expires_at timestamptz, created_at timestamptz not null,
  updated_at timestamptz not null, revision bigint not null default 0
);
...
create table ssh_public_key (
  id uuid primary key, user_id uuid not null references ftp_user(id) on delete cascade,
  fingerprint varchar(255) not null, public_key text not null, note varchar(512), created_at timestamptz not null,
  unique(user_id,fingerprint)
);

create table directory_mapping (
  id uuid primary key, name varchar(128) not null, virtual_path varchar(2048) not null,
  hdfs_path varchar(4096) not null, hdfs_cluster_id varchar(64) not null references hdfs_cluster(id),
...
create table directory_grant (
  id uuid primary key, user_id uuid not null references ftp_user(id) on delete cascade,
  directory_mapping_id uuid not null references directory_mapping(id) on delete cascade,
  access_mode varchar(32) not null, created_at timestamptz not null,
  unique(user_id,directory_mapping_id)
);

create table traffic_policy (
  user_id uuid primary key references ftp_user(id) on delete cascade,
  ...
  time_zone varchar(64) not null default 'UTC', updated_at timestamptz not null
);

create table usage_window (
  user_id uuid not null references ftp_user(id) on delete cascade, direction varchar(16) not null,
  window_start timestamptz not null, window_end timestamptz not null, reserved_files bigint not null default 0,
...
create table config_snapshot (
  id uuid primary key, service_group_id varchar(64) not null references service_group(id), version bigint not null,
  payload_json text not null, payload_sha256 varchar(64) not null, signature text not null,
  status varchar(32) not null, created_by varchar(128) not null, created_at timestamptz not null,
  unique(service_group_id,version)
);

create table transfer_event (
  transfer_id uuid not null, event_sequence bigint not null, user_id uuid not null,
  protocol varchar(16) not null, direction varchar(16) not null, status varchar(32) not null,
  virtual_path varchar(2048), bytes bigint not null default 0, gateway_id varchar(128),
  client_address varchar(128), error_code varchar(64), correlation_id varchar(128), occurred_at timestamptz not null,
...
create table alert_rule (
  id uuid primary key, name varchar(128) not null unique, promql text not null, duration_seconds integer not null,
  severity varchar(32) not null, labels_json text not null default '{}', annotations_json text not null default '{}',
  enabled boolean not null default true, created_at timestamptz not null, updated_at timestamptz not null
);

create table audit_log (
  id uuid primary key, actor varchar(128) not null, action varchar(128) not null, resource_type varchar(64) not null,
  resource_id varchar(128), correlation_id varchar(128), source_address varchar(128),
  before_json text, after_json text, occurred_at timestamptz not null
);

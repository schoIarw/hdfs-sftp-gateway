...
for required in "${MANAGER_JAR}" "${GATEWAY_JAR}" "${SBOM}"; do
  if [[ ! -f "${required}" ]]; then
...
rm -rf "${STAGE_DIR}"
install -d +  "${STAGE_DIR}/bin" +  "${STAGE_DIR}/config" +  "${STAGE_DIR}/systemd" +  "${STAGE_DIR}/keepalived" +  "${STAGE_DIR}/prometheus" +  "${STAGE_DIR}/docker" +  "${STAGE_DIR}/docs"
...
mkdir -p "${DIST_DIR}"
tar --sort=name --mtime="@${SOURCE_EPOCH}" --owner=0 --group=0 --numeric-owner +  -C "${DIST_DIR}" -czf "${ARCHIVE}" "${MEDIA_NAME}"

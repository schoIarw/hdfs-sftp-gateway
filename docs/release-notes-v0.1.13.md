...
- 修复 HDFS 配置替换后的客户端池关闭：旧池拥有的 FileSystem 现在会真正关闭；在途上传事务与读写句柄持有租约，超过 5 分钟的长传输也不会被旧池退休切断。
- 传输并发许可在打开 HDFS 读写流或创建上传暂存文件前取得；方向许可优先于节点总许可，下载方向饱和不再占住上传空闲容量。
- 覆盖上传使用 HDFS 原子覆盖重命名，不再先删除旧目标；同一用户、同一路径的第二次从零上传无法截断已有暂存文件，须先续传或清理残留暂存文件。
...
3. 每用户并发上限默认 16：高并发多段下载客户端（单用户并行流超过 16）如遇到限流，可按业务需要调高 `HFG_MAX_TRANSFERS_PER_USER` 或设为 0 关闭。
   如上传中断后客户端坚持从偏移 0 重新开始，现有 `.uploading/*.part` 会阻止覆盖；应按暂存文件末尾续传，或由运维确认无在途写入后清理该暂存文件。

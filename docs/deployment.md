...
| 文件 | 用途 |
|---|---|
| `hfg-manager-api/target/hfg-manager-api-0.1.6.jar` | Manager、REST API 与管理页面合并包 |
| `hfg-gateway-app/target/hfg-gateway-app-0.1.6.jar` | FTP/SFTP Gateway |
| `hfg-common-contract/target/hfg-common-contract-0.1.6-bootstrap.jar` | 一键生成 CA、Manager 证书和快照密钥 |
...
```bash
docker build -f deploy/docker/Dockerfile.gateway -t hfg-gateway:0.1.6 .
```
